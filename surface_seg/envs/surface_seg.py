...
#             dyn = self._transition_state_search()
            ...
            self.dyn_TS = Sella(self.atoms,  # Your Atoms object
...
#                             reward += 0.5
        ...
        and np.min(minima_differences) > 0.05: # No additional layer allowed
                    self._save_minima()
...
#         self.history['forces'] = self.history['forces'] + [self.forces.tolist()]
    ...
    def save_episode(self):
        save_path = os.path.join(self.history_dir, '%d_%f_%f_%f.npz' %(self.episodes, self.minima['energies'][self.min_idx],                                                                   self.initial_energy, self.highest_energy))
