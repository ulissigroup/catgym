...
#             print('found a new local minima! distance=%1.2f w energy %1.2f'%(np.min(distances), current_energy))
            ...
            reward += max(self.thermal_threshold, self.highest_energy)
        ...
        #otherwise, reset the atoms positions since the minimization didn't do anything interesting
            ...
            self.bad_runs_min += 1
            reward -= self.sigmoid(max(self.thermal_threshold, thermal_ratio)) #* self.bad_runs_min
...
#                 reward -= (self.highest_energy - relative_energy)/self.thermal_energy
        ...
        if np.abs(relative_energy - previous_energy) < 0.01:
            reward -= self.sigmoid(max(self.thermal_threshold, thermal_ratio))
            
#         if relative_energy < 0:
# #             self.negative = True
#             reward += np.abs(self.atoms.get_potential_energy())       
        return reward
...
#                 reward -= np.abs(previous_energy - current_energy)/self.thermal_energy
                self.atoms.positions[self.free_atoms,:]=initial_positions
                self.bad_runs_TS += 1
                reward -= self.sigmoid(max(self.thermal_threshold, thermal_ratio)) ##* self.bad_runs_TS
