...
class MCSEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action_type == 'move':
            ...
            d_reward, early_terminate = self._minimize_and_score()
            reward+=d_reward
            # if early_terminate:
            #     episode_over = True
...
#             reward=1000-current_energy*100+100*np.exp(-np.min(distances))
            ...
            if current_energy > self.highest_energy:
                self.highest_energy = current_energy
            if self.highest_energy/self.thermal_energy > 2:
...
#             reward = 0 
    ...
    def _get_reward(self, episode_finish):
        ...
        # if thermal_ratio>2:
        #     thermal_ratio = 2
        reward -= 15/(1+np.exp(-3*(thermal_ratio - 4.3)))
