...
#             reward += 1
    ...
    def save_traj(self):
        save_path = os.path.join(self.traj_dir, '%d_full.traj' %self.episodes)
        ...
        if self.multi_env:
            if len(self.minima['trajectories']) > 1:
                ...
                for i, atoms in enumerate(self.minima['trajectories']):
                    atoms.set_calculator(EMT())
                    minima.append(atoms)
                write(os.path.join(self.traj_dir, '%d_minima.traj' %self.episodes), minima)
            if len(self.TS['trajectories']) > 0:
                TS = []
                for i, atoms in enumerate(self.TS['trajectories']):
                    atoms.set_calculator(EMT())
                    TS.append(atoms)
                write(os.path.join(self.traj_dir, '%d_TS.traj' %self.episodes), TS)
        else:
...
#             self.highest_energy = current_energy
            if self.highest_energy >= 2*self.thermal_energy:
...
#             reward -= (self.highest_energy - relative_energy)/self.thermal_energy
        ...
        if np.abs(relative_energy - previous_energy) < 0.02:
            reward -= 10*np.abs(relative_energy - previous_energy)/self.thermal_energy

        if relative_energy < 0:
            energy_differences = np.abs(relative_energy-np.array(self.stables))
